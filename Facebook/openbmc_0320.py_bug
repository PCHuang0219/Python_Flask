...
class OpenBMC_0320(Script):
  def __init__(self, dut, job_id="", image_server = 'None'):
    headline = ['Upgrade CPLD for SCM Module']
    purpose = ['To verify if the CPLD of SCM module can be Upgraded by BMC.']
  ...
  def run(self):
    ...
    for i in range(1, self.__cycle + 1):
      # ==================================================================================================
      UI.log('STEP#02 - cycle#' + str(i) + '/' + str(self.__cycle), 'Use scp funtion to copy SCM update image to BMC then check the image is transfer correctlly.')
      new_scm_cpld_ver = self.__TELNET.getImageVersion(self.image_server.local_new_image_path + '/SCMCPLD/version.txt')
      new_scm_cpld_filename = self.__TELNET.getImageFilename(self.image_server.local_new_image_path + '/SCMCPLD/version.txt')
      
      UI.log('SCMCPLD (' + new_scm_cpld_filename + ') New version is "' + new_scm_cpld_ver + '".')
      
      server_file_path = self.image_server.local_new_image_path.replace(self.image_server.local_root_path, '') + '/SCMCPLD/' + new_scm_cpld_filename
      ...
      # ==================================================================================================
      UI.log('STEP#03 - cycle#' + str(i) + '/' + str(self.__cycle), 'Use the command to update SCM CPLD.')
      self.__TELNET.send('sv stop mTerm\r')
      self.__TELNET.expect(self.__dut.ssh_credentials[5])
      self.__TELNET.send('cpld_ver.sh | grep SCMCPLD\r')
      self.__TELNET.expect(self.__dut.ssh_credentials[5])
      self.__TELNET.send('scmcpld_update.sh /var/tmp/minipack_scm_cpld.vme\r')
      ...
      result = re.search('(?i)' + new_scm_cpld_ver, self.__TELNET.getBuff(), re.M)
    ...
    if self.__fail_count == 0:
      UI.log('PASS', 'Cycle#' + str(i) + '/' + str(self.__cycle) + ': BMC_0320 Upgrade_CPLD_for_SCM_Module is passed.')
    else:
      UI.log('FAIL', 'Cycle#' + str(i) + '/' + str(self.__cycle) + ': BMC_0320 Upgrade_CPLD_for_SCM_Module is failed.')
