...
class OpenBMC_0390(Script):
  def __init__(self, dut, job_id="", image_server = 'None'):
    headline = ['Downgrade CPLD for SMB Board']
    purpose = ['To verify if the CPLD of SMB board can be downgraded by BMC.']
  ...
  def run(self):
    ...
    self.__cycle = 1
    self.__fail_count = 0
    self.__old_smb_cpld_ver = 'SMBCPLD: 2.5'
    self.__old_smb_cpld_filename = 'smb_cpld_3A04_V2.5_20180820_1000.vme'
   # ==================================================================================================
    ...
    for i in range(1, self.__cycle + 1):
      # ==================================================================================================
      UI.log('STEP#02 - cycle#' + str(i) + '/' + str(self.__cycle), 'Use scp funtion to copy SMB downgrade image to BMC then check the image is transfer correctlly.')
      server_file_path = self.image_server.local_old_image_path.replace(self.image_server.local_root_path, '') + '/' + self.__old_smb_cpld_filename
      
      UI.log('SMB CPLD (' + self.__old_smb_cpld_filename + ') Downgrade version is "' + self.__old_smb_cpld_ver + '".')
      
      self.__TELNET.downloadFile(self.__fb.scp_server_username, self.__fb.scp_server_password, self.__fb.scp_server_ip,
        server_file_path, '/var/tmp/minipack_smb_old_cpld.vme')
        
      # ==================================================================================================
      UI.log('STEP#03 - cycle#' + str(i) + '/' + str(self.__cycle), 'Use the command to downgrade SMB CPLD.')
      self.__TELNET.send('sv stop mTerm\r')
      self.__TELNET.expect(self.__dut.ssh_credentials[5])
      self.__TELNET.send('cpld_ver.sh | grep SMBCPLD\r')
      self.__TELNET.expect(self.__dut.ssh_credentials[5])
      self.__TELNET.send('smbcpld_update.sh /var/tmp/minipack_smb_old_cpld.vme\r')
      ...
      result = re.search('(?i)' + self.__old_smb_cpld_ver, self.__TELNET.getBuff(), re.M)
    ...
    if self.__fail_count == 0:
      UI.log('PASS', 'Cycle#' + str(i) + '/' + str(self.__cycle) + ': BMC_0390 Downgrade_CPLD_for_SMB_Board is passed.')
    else:
      UI.log('FAIL', 'Cycle#' + str(i) + '/' + str(self.__cycle) + ': BMC_0390 Downgrade_CPLD_for_SMB_Board is failed.')
