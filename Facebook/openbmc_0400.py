...
class OpenBMC_0400(Script):
  def __init__(self, dut, job_id="", image_server = 'None'):
    headline = ['Downgrade CPLD for FCM']
    purpose = ['To verify if the CPLD of FCM board can be downgraded by BMC.']
  ...
  def run(self):
    ...
    self.__cycle = 1
    self.__fail_count = 0
    self.__old_fcm_cpld_ver = {}
    self.__old_fcm_cpld_ver[0] = 'Top FCMCPLD: 1.0'
    self.__old_fcm_cpld_ver[1] = 'Bottom FCMCPLD: 1.0'
    self.__old_fcm_cpld_filename = 'Minipack_FCM_V1.0_20180620.vme'
    ...
    for i in range(1, self.__cycle + 1):
      # ==================================================================================================
      UI.log('STEP#02 - cycle#' + str(i) + '/' + str(self.__cycle), 'Use scp funtion to copy FCM downgrade image to BMC then check the image is transfer correctlly.')
      server_file_path = self.image_server.local_old_image_path.replace(self.image_server.local_root_path, '') + '/' + self.__old_fcm_cpld_filename
      
      UI.log('FCM CPLD (' + self.__old_fcm_cpld_filename + ') Downgrade top version is "' + self.__old_fcm_cpld_ver[0] + '".')
      UI.log('FCM CPLD (' + self.__old_fcm_cpld_filename + ') Downgrade button version is "' + self.__old_fcm_cpld_ver[1] + '".')
      
      self.__TELNET.downloadFile(self.__fb.scp_server_username, self.__fb.scp_server_password, self.__fb.scp_server_ip,
        server_file_path, '/var/tmp/minipack_fcm_old_cpld.vme')
        
      # ==================================================================================================
      UI.log('STEP#03 - cycle#' + str(i) + '/' + str(self.__cycle), 'Use the command to downgrade FCM CPLD.')
      ...
      self.__TELNET.expect(self.__dut.ssh_credentials[5])
      self.__TELNET.send('fcmcpld_update.sh /var/tmp/minipack_fcm_old_cpld.vme\r')
      ...
      result = re.search('(?i)' + self.__old_fcm_cpld_ver[0], self.__TELNET.getBuff(), re.M)
      ...
      result = re.search('(?i)' + self.__old_fcm_cpld_ver[1], self.__TELNET.getBuff(), re.M)
    ...
    if self.__fail_count == 0:
      UI.log('PASS', 'Cycle#' + str(i) + '/' + str(self.__cycle) + ': BMC_0400 Downgrade_CPLD_for_FCM is passed.')
    else:
      UI.log('FAIL', 'Cycle#' + str(i) + '/' + str(self.__cycle) + ': BMC_0400 Downgrade_CPLD_for_FCM is failed.')
